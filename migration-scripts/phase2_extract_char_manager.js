#!/usr/bin/env node
/**
 * Phase 2: Extract character-manager functions from script.js
...
const OUTPUT_PATH = path.join(__dirname, 'char_manager_extract.json');
...
// Functions to extract into character-manager.js
...
const EXTRACT_PRIVATE = [
    ...
    'updateAlternateGreetingsHintVisibility',
...
for (const node of ast.body) {
    ...
    if (name && ALL_TARGETS.has(name)) {
        ...
        if (node.leadingComments) {
            docStartLine = node.leadingComments[0].loc.start.line;
        } else {
